...
var Game = {
  ...
  beginNewGame: function (mode) {
    ...
    return Promise.resolve().then(function () {
      ...
      if (mode === 'new') {
        ...
        self._targetStage = 's2'; self._fromCp = false;
      ...
      return Assets.loadCritical(function (p) {
        self.loadStep(0.12 + p * 0.23, '원생생물 연못을 불러오는 중…');
      });
    }).then(function (res) {
      ...
      return self.tick(0.38, '원생생물 연못을 불러오는 중…');
    }).then(function () {
  ...
  buildEnding: function () {
    ...
    $('#ending-cases').innerHTML = [
      { i: 'biotech_protist_biofuel', t: '<b>원생생물</b>(해캄·짚신벌레)을 이용해 생물 연료를 만들기도 해요.' }
    ].map(function (c) {
    ...
    ['spirogyra', 'paramecium', 'pondwater'].forEach(function (k) {
      var c = Level.CARDS[k];
      if (Save.has('cards', k)) {
